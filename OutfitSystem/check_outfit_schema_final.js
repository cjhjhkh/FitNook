...
async function checkAndFix() {
    ...
    try {
        ...
        if (!columns.includes('image_url')) {
            ...
            // 如果 description 也不存在，就加在 weather 后面
            const afterCol = columns.includes('description') ? 'description' : 'weather';
            await connection.query(`ALTER TABLE outfits ADD COLUMN image_url VARCHAR(500) DEFAULT "" COMMENT "合成图URL" AFTER ${afterCol}`);
